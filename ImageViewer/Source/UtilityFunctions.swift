...
func degreesToRadians(_ degree: CGFloat) -> CGFloat { return CGFloat(Double.pi) * degree / 180 }
...
func rotationAdjustedBounds() -> CGRect {
    ...
    if UIApplication.isPortraitOnly {
        return (UIDevice.current.orientation.isLandscape)
            ? CGRect(origin: CGPoint.zero, size: window.bounds.size.inverted()) : window.bounds
    }
